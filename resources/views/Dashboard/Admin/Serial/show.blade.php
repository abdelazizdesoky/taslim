                    ...
                    <div class="vtimeline">
                        @if (isset($invoices) && $invoices->count() > 0)
                            @foreach ($invoices as $invoice)
                                <div
                                    class="timeline-wrapper  {{ $invoice->invoice_type == 1 ? 'timeline-wrapper-success ' : 'timeline-inverted  timeline-wrapper-info' }} ">
                                    <div class="timeline-badge {{ $invoice->invoice_type == 1 ? 'success ' : 'info' }}"><img
                                            class="timeline-image" alt="img"
                                            src="{{ URL::asset('dashboard/img/faces/3.jpg') }}"> </div>
                                    <div class="timeline-panel">
                                        <div class="timeline-heading">
                                            <h6 class="timeline-title"><a
                                                    href="{{ route('admin.invoices.show', $invoice->id) }}">{{ $invoice->code }}
                                            </h6></a>
                                        </div>
                                        <div class="timeline-body">
                                            ...
                                            <p>
                                                @if ($invoice->invoice_type == 2)
                                                    ...
                                                    {{ $invoice->customer->name ?? '-' }}-{{ $invoice->customer->code ??'-' }} <!-- اسم العميل -->
                                        ...
                                        <div class="timeline-footer d-flex align-items-center flex-wrap">
                                            ...
                                            <span class="mr-auto"><i
                                                    class="fe fe-calendar text-muted mr-1"></i>{{ $invoice->invoice_date }}
                                                - </span>
